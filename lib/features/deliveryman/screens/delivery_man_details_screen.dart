...
import 'package:gazzer_vendorapp/features/deliveryman/widgets/amount_card_widget.dart';
...
class DeliveryManDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (Get.find<ProfileController>()
        ...
        'subscription') {
      haveSubscription =
          Get.find<ProfileController>().profileModel!.subscription!.review == 1;
    } else {
          ...
          Expanded(
            ...
            child:
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                haveSubscription
                    ? dmController.dmReviewList != null
                        ? dmController.dmReviewList!.isNotEmpty
                            ? ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  return ReviewWidget(
                                    review: dmController.dmReviewList![index],
                                    fromRestaurant: false,
                                    hasDivider: index !=
                                        dmController.dmReviewList!.length - 1,
                                  );
                                },
                              )
                            : Padding(
          ...
          CustomButtonWidget(
            onPressed: () {
              Get.dialog(ConfirmationDialogWidget(
                ...
                onYesPressed: () =>
                    dmController.toggleSuspensionDeliveryMan(deliveryMan.id),
              ));
            },
            buttonText: dmController.isSuspended
                ? 'un_suspend_this_delivery_man'.tr
                : 'suspend_this_delivery_man'.tr,
            margin: const EdgeInsets.all(Dimensions.paddingSizeSmall),
            color: dmController.isSuspended
                ? Colors.green
                : Theme.of(context).colorScheme.error,
          ),
        ]);
