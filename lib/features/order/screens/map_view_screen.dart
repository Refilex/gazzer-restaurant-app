...
import 'package:gazzer_vendorapp/features/order/widgets/location_search_dialog_widget.dart';
...
class _MapViewScreenState extends State<MapViewScreen> {
  ...
  void initState() {
    Get.find<DeliveryManController>()
        .selectDeliveryManInMap(null, canUpdate: false);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: widget.deliveryManList != null
          ? Stack(children: [
              GoogleMap(
                initialCameraPosition: CameraPosition(
                    target: LatLng(
                      double.parse(widget.deliveryManList!.first.lat!),
                      double.parse(widget.deliveryManList!.first.lng!),
                    ),
                    zoom: 16),
                  ...
                  child: PageView.builder(
                    onPageChanged: (int index) {
                      LatLng latLng = LatLng(
                          double.parse(widget.deliveryManList![index].lat!),
                          double.parse(widget.deliveryManList![index].lng!));
                      if (_controller != null) {
                      ...
                      _customInfoWindowController.addInfoWindow!(
                          MapCustomInfoWindowWidget(
                              image:
                                  '${Get.find<SplashController>().configModel!.baseUrls!.deliveryManImageUrl}/${widget.deliveryManList![index].image!}'),
                          latLng);
                    },
  ...
  Future<void> setMarker(List<DeliveryManListModel> deliveryManList) async {
    try {
      ...
      LatLngBounds? bounds;
      if (_controller != null) {
        if (double.parse(deliveryManList.first.lat!) <
            double.parse(deliveryManList.last.lat!)) {
          bounds = LatLngBounds(
            southwest: LatLng(double.parse(deliveryManList.first.lat!),
                double.parse(deliveryManList.first.lng!)),
            northeast: LatLng(double.parse(deliveryManList.last.lat!),
                double.parse(deliveryManList.last.lng!)),
          );
        } else {
          bounds = LatLngBounds(
            southwest: LatLng(double.parse(deliveryManList.last.lat!),
                double.parse(deliveryManList.last.lng!)),
            northeast: LatLng(double.parse(deliveryManList.first.lat!),
                double.parse(deliveryManList.first.lng!)),
          );
        }
      }
      ...
      for (int i = 0; i < deliveryManList.length; i++) {
        LatLng latLng = LatLng(double.parse(deliveryManList[i].lat!),
            double.parse(deliveryManList[i].lng!));
        ...
        _markers.add(Marker(
          ...
          onTap: () {
            _customInfoWindowController.addInfoWindow!(
                MapCustomInfoWindowWidget(
                    image:
                        '${Get.find<SplashController>().configModel!.baseUrls!.deliveryManImageUrl}/${deliveryManList[i].image}'),
                latLng);
          },
  ...
  Widget deliveryManCard(DeliveryManListModel deliveryManListModel) {
    return InkWell(
      onTap: () {
        LatLng latLng = LatLng(double.parse(deliveryManListModel.lat!),
            double.parse(deliveryManListModel.lng!));
        if (_controller != null) {
        ...
        _customInfoWindowController.addInfoWindow!(
            MapCustomInfoWindowWidget(
                image:
                    '${Get.find<SplashController>().configModel!.baseUrls!.deliveryManImageUrl}/${deliveryManListModel.image}'),
            latLng);
      },
      child: Container(
