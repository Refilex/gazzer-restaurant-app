...
class SubscriptionController extends GetxController implements GetxService {
    ...
    if (response.statusCode == 200) {
      if (response.body['redirect_link'] != null) {
        ...
        if (GetPlatform.isWeb) {
          ...
          Get.toNamed(
              RouteHelper.getPaymentRoute(digitalPaymentName, redirectUrl));
        }
      } else {
        _renewStatus = 'packages';
  ...
  Future<void> getPackageList() async {
    if (Get.find<BusinessController>().packageModel == null ||
        Get.find<BusinessController>().packageModel!.packages!.isEmpty) {
